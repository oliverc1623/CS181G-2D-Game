...
fn restitute(statics: &[Wall], dynamics: &mut [Mobile], contacts: &mut [Contact]) {
    ...
    for c1 in contacts.iter() {
        ...
        let player_indx = {
            if let ColliderID::Dynamic(x) = c1.a {
                x
            } else {
                0
            }
        ...
        // horizontal < vertical
        let horizontal_mtv = c1.mtv.0;
        let vertical_mtv = c1.mtv.1;
        let player_rect = &dynamics[player_indx];

        if disp[player_indx].0 == horizontal_mtv || disp[player_indx].1 == vertical_mtv {
            continue;
        }

        if horizontal_mtv < vertical_mtv {
            if player_rect.rect.x < collided_wall.rect.x {
                println!("Box touched left side{:?}", c1);
                dynamics[player_indx].rect.x -= horizontal_mtv;
                disp[player_indx] = (horizontal_mtv, 0)
            } else {
                println!("Box touched right side{:?}", c1);
                dynamics[player_indx].rect.x += horizontal_mtv;
                disp[player_indx] = (horizontal_mtv, 0)
            }
        } else {
            if player_rect.rect.y < collided_wall.rect.y {
                println!("Box touched down side{:?}", c1);
                dynamics[player_indx].rect.y -= vertical_mtv;
                disp[player_indx] = (0, vertical_mtv)
            } else {
                println!("Box touched up side{:?}", c1);
                dynamics[player_indx].rect.y += vertical_mtv;
                disp[player_indx] = (0, vertical_mtv)
            }
        }
